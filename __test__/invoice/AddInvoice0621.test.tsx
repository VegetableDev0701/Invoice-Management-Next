// mocked data
import projectSummaryJSON from '@/__test__/invoiceMocks/0621/projectSummary.json';
import budgetTotalsV2JSON from '@/__test__/invoiceMocks/0621/budgetTotalsV2.json';
import costCodeNameListJSON from '@/__test__/invoiceMocks/0621/costCodeNameList.json';
import projectInvoicesJSON from '@/__test__/invoiceMocks/0621/projectInvoices.json';
import projectLaborFeesJSON from '@/__test__/invoiceMocks/0621/projectLaborFees.json';
import currentActualsJSON from '@/__test__/invoiceMocks/0621/currentActuals.json';
import currentActualChangeOrdersJSON from '@/__test__/invoiceMocks/0621/currentActualsChangeOrders.json';
...
describe('testing the invoice build process', () => {
  test('test createBudgetActualsObject output', () => {
    ...
    expect(Object.keys(currentActuals).length).toBe(10);
    ...
    expect(Object.keys(invoiceCurrentActuals.laborFee).length).toBe(5);
  });
  test('test budget totals and profit and taxes calculations', () => {
    ...
    expect(budgetedTotals.total).toBe('22,065.31');
    expect(changeOrderTotals.total).toBe('0.00');
    expect(profitTaxesObject.profit).toBe(2647.84);
    expect(profitTaxesObject.liability).toBe(243.67);
    expect(profitTaxesObject.boTax).toBe(124.78);
    expect(profitTaxesObject.salesTax).toBe(2533.24);
    expect(profitTaxesObject.total).toBe(27614.84);
