  ...
  if (req.method === 'PATCH') {
    try {
      ...
      const response = await fetch(
        // `${api()}/${companyId}/remove-invoices-from-change-order?project_id=${projectId}&change_order_id=${changeOrderId}`,
        `${getAPIUrl()}/${companyId}/remove-invoices-from-change-order?project_id=${projectId}`,
        {
          method: req.method,
          body: req.body,
          headers: {
