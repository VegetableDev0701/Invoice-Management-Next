...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const isOnlyCurrentActuals = req.headers.isonlycurrentactuals as string;
  ...
  if (req.method === 'GET') {
    try {
      ...
      const response = await fetch(
        `${getAPIUrl()}/${companyId}/get-client-bill?project_id=${projectId}&client_bill_id=${clientBillId}&is_only_current_actuals=${isOnlyCurrentActuals}`,
        {
          method: req.method,
          headers: {
