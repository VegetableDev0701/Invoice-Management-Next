...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const changeStatusTo = req.headers.changestatusto as string;
  try {
    if (req.method !== 'PATCH') {
      res.status(405).json({ error: 'Wrong method, PATCH only.' });
      return;
    }
    ...
    const response = await fetch(
      `${getAPIUrl()}/${companyId}/change-project-status?change_status_to=${changeStatusTo}`,
      {
        method: req.method,
        body: req.body,
        headers: {
